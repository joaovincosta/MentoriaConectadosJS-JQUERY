...
function formatarArray(meuArray) {
  var matriz = []
  for (var i = 0; i < meuArray.length; i++) {
    matriz[i] = meuArray[i].split(' ').map(function(item) {
...
function calcularSomaDiagonalPrincipal(matriz) {
  ...
  for (var i = 0; i < matriz.length; i++) {
    ...
    soma += numero
    resultados += `Número: ${numero}, Soma acumulada: ${soma}<br>`
  }

  document.getElementById('resultados').innerHTML = `
    <h3>Números encontrados na diagonal principal:</h3>
    ${resultados}
    <p><strong>Total da soma: ${soma}</strong></p>
  `;
}

var matrizFormatada = formatarArray(meuArray)
calcularSomaDiagonalPrincipal(matrizFormatada)
